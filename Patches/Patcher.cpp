...
{
  size_t result = 0;
  const auto*const pDosHeader = (GetModuleFromAddress(hModule) != NULL) ? static_cast<const IMAGE_DOS_HEADER*>(hModule)
                                                                        : nullptr;
  ...
  uint8     offsetLut[MaxOverwriteSize])
{
  ...
  for (size_t i = 0; ((foundEnd == false) && (i < count)); ++i) {
    ...
    case X86_INS_LOOP:  case X86_INS_LOOPE:  case X86_INS_LOOPNE:  case X86_INS_JCXZ:  case X86_INS_JECXZ:
      ...
      else if (pInsns[i].bytes[0] >= 0xE0 && pInsns[i].bytes[0] <= 0xE3) {
        // Workaround for Capstone issue where JE sometimes identifies as JCXZ.
  ...
  uint8    offsetLut[MaxOverwriteSize] = nullptr) // [out] LUT of overwritten instruction offsets to trampoline offsets.
{
  ...
  if (csOpened) {
    const cs_err csError = cs_close(&hDisassembler);
    if (status == Status::Ok) {
  ...
  uint32                        options)
{
  ...
  static constexpr Insn     PushInsns[] = {
    //   Rax:    Rcx:    Rdx:    Rbx:    Rsi:    Rdi:    Rbp:    R8:                R9:                R10:
        {0x50}, {0x51}, {0x52}, {0x53}, {0x56}, {0x57}, {0x55}, {{0x41, 0x50}, 2}, {{0x41, 0x51}, 2}, {{0x41, 0x52}, 2},
    //   R11:               R12:               R13:               R14:               R15:               Rsp:    Rflags:
        {{0x41, 0x53}, 2}, {{0x41, 0x54}, 2}, {{0x41, 0x55}, 2}, {{0x41, 0x56}, 2}, {{0x41, 0x57}, 2}, {0x54}, {0x9C}
  };
  static constexpr Insn     PopInsns[]  = {
        {0x58}, {0x59}, {0x5A}, {0x5B}, {0x5E}, {0x5F}, {0x5D}, {{0x41, 0x58}, 2}, {{0x41, 0x59}, 2}, {{0x41, 0x5A}, 2},
        {{0x41, 0x5B}, 2}, {{0x41, 0x5C}, 2}, {{0x41, 0x5D}, 2}, {{0x41, 0x5E}, 2}, {{0x41, 0x5F}, 2}, {0x5C}, {0x9D}
  };
  ...
  if (registers.empty() == false) {
    ...
    for (uint32 i = 0, mask = byRefMask; BitScanFwdIter(&i, mask); mask &= ~(1 << i)) {
      ...
      registersByRefMask |= (1u << static_cast<uint32>(registers[i]));
    }
...
#pragma pack(pop)
    ...
    if (BitFlagTest(registersByRefMask, 1 << static_cast<uint32>(StackRegister))) {
      assert(false);  // ** TODO Implement overwriting ESP in the custom return destination path.
      if (stackRegIndex == 0) {
        // ...
      }
      else {
...
Status PatchContext::EditImports(
  const std::vector<ImportInfo>&  importInfos)
{
